...
start_day = 366+365
end_day = 731+365
data = load_data('New_data/Matrix_Intraday3_none.csv')
...
for window in windows:
    ...
    for h in range(24):
        ...
        X3 = data_day['-FL'].values.tolist()
        X4 = data_day['-FW'].values.tolist()
        X5 = data_day['-FS'].values.tolist()
        X6 = data_day['-FL_1d'].values.tolist()
        X7 = data_day['-FW_1d'].values.tolist()
        X8 = data_day['-FS_1d'].values.tolist()
        ...
        for d in range(start_day,end_day):  # Adjust the range as needed 731
            ...
            X = np.column_stack([X0_subset, X1_subset, X2_subset, X3_subset, X4_subset, X5_subset, X6_subset, X7_subset, X8_subset])
            betas, _, _, _ = np.linalg.lstsq(X, Y_subset, rcond=None)
            X_fut = np.array([1, X1[d], X2[d], X3[d], X4[d], X5[d], X6[d], X7[d], X8[d]])
...
print(returning)
returning.to_csv('New_data/Forecast_Intraday3_Real.csv', index=False, header = None)
