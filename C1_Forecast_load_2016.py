    ...
    for h in range(24):
        ...
        if window==365:
            for d in range(start_day, end_day):  # Adjust the range as needed 731
                ...
                X8_subset = X8[d+6 - window:d]
                X9_subset = X9[d+6 - window:d]
                X10_subset = X10[d+6 - window:d]
                X11_subset = X11[d+6 - window:d]
                X12_subset = X12[d+6 - window:d]
                ...
                #rok 2016
                # psuje: 5,6 wiec dajemy d+6 bo pokjawiaja sie Nany
                # Wyniki: 7,9: 1.104;
                # Wyniki: 8,11: 1,106;
                # Wyniki: 10,12: 1,105
                # arr1 = np.linalg.inv(np.dot(X.T, X))  #opcja z zajec
                # betas = np.dot(arr1, np.dot(X.T, Y_subset)) #opcja z zajec
                # betas = stats.linregress(X,Y_subset).slope #opcja 2
                # betas = stats.linregress()
                betas, _, _, _ = np.linalg.lstsq(X, Y_subset, rcond=None) #opcja 3
...
df = pd.DataFrame({
    ...
    '365': forecasts_window_lists[5], # o 7 pierwszych za malo
})
print(df)
...
#new_df.to_csv('New_data/Forecast_load_enhanced_2016.csv', index=False,  header=False)
