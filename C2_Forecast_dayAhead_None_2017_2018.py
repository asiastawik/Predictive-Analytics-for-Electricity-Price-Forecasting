...
start_day= 366
end_day = 731+365
data = load_data('New_data/Matrix_dayAhead_None.csv')
...
for window in windows:
    ...
    for h in range(24):
        ...
        X1 = data_day['DA-1d'].values.tolist()
        X2 = data_day['DA-2d'].values.tolist()
        X3 = data_day['DA-7d'].values.tolist()
        X4 = data_day['DA_avg'].values.tolist()
        X5 = data_day['DA_min'].values.tolist()
        X6 = data_day['DA_max'].values.tolist()
        X7 = data_day['DA_last'].values.tolist()
        Y = data_day[['DayAhead']].values.tolist()
        for d in range(start_day,end_day):  # Adjust the range as needed 731
            Y_subset = Y[d+6 - window:d]
            X0_subset = X0[d+6 - window:d]
            X1_subset = X1[d+6 - window:d]
            X2_subset = X2[d+6 - window:d]
            X3_subset = X3[d+6 - window:d]
            X4_subset = X4[d+6 - window:d]
            X5_subset = X5[d+6 - window:d]
            X6_subset = X6[d+6 - window:d]
            X7_subset = X7[d+6 - window:d]
            X = np.column_stack([X0_subset, X1_subset, X2_subset, X3_subset, X4_subset, X5_subset, X6_subset, X7_subset])
            betas, _, _, _ = np.linalg.lstsq(X, Y_subset, rcond=None)
            X_fut = np.array([1, X1[d], X2[d], X3[d], X4[d], X5[d], X6[d], X7[d]])
...
new_df.to_csv('New_data/Forecast_dayAhead_none_2017_2018.csv', index=False,  header=False)
